				...
				if (IGM_index.le.5000000) then
			 		...
			 		if ((age.eq.1).and.(age_son(paneli).lt.31)) then 
		  			...
		  			if ((age.ge.11).and.(age.le.31)) then 
		  				k_igm = min(currenta+theta*(xz_grid(panelx(paneli),panelz(paneli))*currenta)**mu,&
  ...
  !   !  			endif 
		! 		! endif 
				...
				if (simutime.ge.(MaxSimuTime-9)) then 
					k_igm     = min(panela(paneli)+theta*(xz_grid(panelx(paneli),panelz(paneli))*panela(paneli))**mu,&
		    					& (mu*P*xz_grid(panelx(paneli),panelz(paneli))**mu/(R+DepRate))**(1.0_dp/(1.0_dp-mu)) )
		...
		DO paneli=1,totpop
		    ...
		    panelK(paneli)    = min(currenta+theta*(xz_grid(currentxi,currentzi)*currenta)**mu,&
...
SUBROUTINE  Simulation_Life_Cycle_Patterns(bench_indx)
   ...
   ! Capital
   		Panel_k(i,1,i_z) = min(Panel_a(i,1,i_z)+theta*(xz_grid(Panel_x(i,1,i_z),i_z)*Panel_a(i,1,i_z))**mu , &
