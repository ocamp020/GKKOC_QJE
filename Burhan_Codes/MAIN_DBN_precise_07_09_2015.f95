...
!	REAL(DP), PARAMETER  :: rho_lambda=0.5_DP, sigma_lambda_eps=0.34_DP 
	...
	REAL(DP), PARAMETER  :: tauWmin=0.015_DP, tauWinc=0.005_DP
...
MODULE global
    ...
    REAL(DP), DIMENSION(nlambda)                  :: phi_lambda

	! investment abilities
	REAL(DP), DIMENSION(nz)                  :: zgrid , Gz, cdf_Gz
	REAL(DP), DIMENSION(nz,nz)             :: pr_z, cdf_pr_z
	
	REAL(DP), DIMENSION(nlambda) :: RetY  ! Retirement income, the first 45 years are assigned zero. 
...
! FOR EACH TAX RATE WE WILL HAVE DIFFERENT Y GRIDS
    ...
    REAL(DP), DIMENSION(MaxAge, na, nz, nlambda, ne) ::DBN1
    ...
    REAL(DP) :: prct1_wealth, prct10_wealth, YBAR, GBAR, GBAR_exp,GBAR_exp_old, GBAR_bench, Y_bench, Y_exp
...
cprime =   Linear_Int(Ygrid(:,zi), &
                                    & RetCons(age+1,:,zi,lambdai),na, yprime)    


FOC_R   = (1.0_DP / (YGRID(ai,zi)  + RetY(lambdai) - aprimet )  &
...
IMPLICIT NONE   
real(DP):: aprimet, ntemp, MBaprime, FOC_W,  yprime, exp1overcprime
integer:: epindx
...
!print*,'geldi'
!print*,'EndoYgrid(age+1,:,zi,lambdai,epindx)'
!print*,EndoYgrid(age+1,:,zi,lambdai,epindx)
!print*,''
!print*,'EndoCons(age+1,:,zi,lambdai,epindx)='
!print*,EndoCons(age+1,:,zi,lambdai,epindx)
!print*,''
!pause
...
!DO epindx=1,ne
!
!      cprime(epindx)  =   Linear_Int( EndoYgrid(age+1,:,zi,lambdai,epindx),&
!                                    & EndoCons(age+1,:,zi,lambdai,epindx), na,    yprime  )
!ENDDO
...
!
!OPEN   (UNIT=2, FILE='DBN_bench')
!READ(unit=2, FMT=*)  DBN1
!CLOSE (unit=2)  
!
!
!OPEN   (UNIT=2, FILE='vdbn' , STATUS='replace')
!DO age=1,MaxAge 
!DO ai=1,na    
!    DO zi=1,nz
!        DO lambdai=1,nlambda          
!             DO ei=1,ne
!                  WRITE  (UNIT=2, FMT=*) DBN1(age, ai, zi, lambdai, ei)
!               ENDDO ! ei          
!        ENDDO ! lambdai
!    ENDDO ! zi
!ENDDO ! ai
...
!tauL_exp = tauL
!
!! the following are eq with 201 grids
! EBAR_bench=  1.37578920056420
! rr_bench=  7.644455552458888E-002
! wage_bench=  1.52485921462381
! EBAR_exp=  1.41872902812391
! rr_exp=  7.401449382417461E-002
! wage_exp=  1.55466995103491
!tauW_exp =  1.994021138518702E-002
!
!
!! the following are eq with 101 grids
! EBAR_bench=  1.37875888006283
! rr_bench=  7.648234505804212E-002
! wage_bench=  1.52462674330453
! EBAR_exp=  1.42085715437523
! rr_exp=  7.406393553049710E-002
! wage_exp=  1.55422938740978
!tauW_exp= 1.981791251542411E-002
!
!CALL COMPUTE_WELFARE_GAIN 
!
!
!PRINT*,'Welfare Gain Ended'
!PAUSE
!
...
!beta=0.9515_DP
!phi=1.25_DP
!sigma_lambda_eps=0.34_DP
!rho_z= 0.50651_DP
!sigma_z_eps=0.624_DP 

!OPEN   (UNIT=2, FILE='param1')
!DO parindx1=1,10
!    READ(unit=2, FMT=*) beta, rho_z, sigma_z_eps, sigma_lambda_eps,  phi
!    CALL  INITIALIZE
!    CALL FIND_DBN_EQ
!    CALL COMPUTE_STATS
!ENDDO    
...
print*,'---------------------------'
print*,''
print*,'Output Gain Prct=', 100.0_DP*(Y_exp/Y_bench-1.0), 'Average Welfare Gain prct=',100.0_DP*sum(Cons_Eq_Welfare*DBN1)
print*,''
print*,'---------------------------'
...
print*,'Av. Welfare Gain by age (prct)='
DO age=MaxAge,1,-1
    Cons_Eq_Welfare(age,:,:,:,:)=exp((ValueFunction_exp(age,:,:,:,:)-ValueFunction_Bench(age,:,:,:,:))/CumDiscountF(age))-1.0_DP
    print*, 100*sum(Cons_Eq_Welfare(age,:,:,:,:)*DBN1(age,:,:,:,:))/sum(DBN1(age,:,:,:,:))
ENDDO


print*,'---------------------------'
print*,''
print*,'Average Welfare Gain Whole Population (prct)=',100.0_DP*sum(Cons_Eq_Welfare*DBN1)
print*,''
print*,'---------------------------'
    ...
    DO zi=1,nz
        DO lambdai=1,nlambda          
              DO ei=1,ne            
                    ...
                    & 1.0_DP/RetCons(age+1, 1, zi, lambdai) , 1.0_DP/RetCons(age+1, na, zi, lambdai) , ValueP2)  
...
use  programfunctions
IMPLICIT NONE
INTEGER:: tklo, tkhi, age1, age2, z1, z2, a1, a2, lambda1, lambda2, e1, e2, DBN_iter, simutime
...
!    print*,'simutume=', simutime 
!    CALL COMPUTE_STATS
!    print*,'DBN_dist=',DBN_dist, 'QBAR=', QBAR ,  'NBAR=', NBAR , &
!             & 'YBAR=', YBAR ,  'rr=', rr, 'wage=', wage, 'Ebar=',Ebar , &
!            & 'prtct1=  ', 1.0_DP-cdf_tot_a_by_prctile(99)/cdf_tot_a_by_prctile(100), &
!            & 'prtct10=', 1.0_DP-cdf_tot_a_by_prctile(90)/cdf_tot_a_by_prctile(100)
...
! compute analytical solution on exogenous grids
call ANALYTIC_SOLUTION_MU1(1, 1, 1, 1, tempvar1, tempvar2, tempvar3)
...
DO lambdai=1,nlambda
    DO zi=1,nz
        DO ai=1,na
                RetCons(age,ai,zi,lambdai) =  YGRID(ai, zi) + RetY(lambdai) 
          ENDDO ! ai
    ENDDO ! zi
ENDDO ! lambdai
...
!    print*,'age=',age
    DO lambdai=1,nlambda
        DO zi=1,nz
              DO ai=1,na
                    EndoCons(ai) =   RetCons(age+1, ai, zi, lambdai)/( beta*survP(age)*MBGRID(ai,zi))    
                    EndoYgrid(ai) =  agrid(ai) +  EndoCons(ai)   - RetY(lambdai)
              ENDDO ! ai
...
!              ENDDO ! ai  
              ...
              DO ai=tempai,na               
                    ...
                    Aprime(age, ai, zi, lambdai,:) = YGRID(ai,zi)+RetY(lambdai)  &
                                        & -  RetCons(age, ai, zi, lambdai)                                         
                       If   (Aprime(age, ai, zi, lambdai,1)  .lt. amin) then
                              Aprime(age, ai, zi, lambdai,1)=amin
                              RetCons(age, ai, zi, lambdai) =  YGRID(ai,zi)  + RetY(lambdai)  - Aprime(age, ai, zi, lambdai,1) 
                               IF (RetCons(age, ai, zi, lambdai) .le. 0.0_DP)  THEN
                                    print*,'r1: RetCons(age, ai, zi, lambdai)=',RetCons(age, ai, zi, lambdai)
                                ENDIF                   
                        endif                                                         
              ENDDO ! ai  
...
!                      ap_temp    = Aprime(age, tempai, zi, lambdai,1) 
                      brentvalue = brent(min(amin,YGRID(ai,zi)), (amin+YGRID(ai,zi))/2.0_DP , YGRID(ai,zi)+RetY(lambdai), &
                                        & FOC_R, brent_tol, Aprime(age, ai, zi, lambdai,1))

                      RetCons(age, ai, zi, lambdai) =  YGRID(ai,zi)  + RetY(lambdai)  - Aprime(age, ai, zi, lambdai,1)
                      Aprime(age, ai, zi, lambdai,:) =Aprime(age, ai, zi, lambdai,1)
                      IF (RetCons(age, ai, zi, lambdai) .le. 0.0_DP)  THEN
                            print*,'r2: RetCons(age, ai, zi, lambdai)=',RetCons(age, ai, zi, lambdai)
                       ENDIF                   
                      ai = ai + 1
               ENDDO  
...
!  Transfer consumption for period RetAge
...
DO age=MaxAge,RetAge,-1
  DO lambdai=1,nlambda
        DO zi=1,nz
            DO ai=1,na
!                DO ei=1,ne
                    Cons(age,ai,zi,lambdai,:) = RetCons(age,ai,zi,lambdai)  
!                 ENDDO
            ENDDO
        ENDDO
    ENDDO
        ...
        DO zi=1,nz
            DO ei=1,ne
                ...
                DO ai=1,na
                      ...
                      EndoHours(ai) = max(0.0_DP, &
                                        &   1.0_DP-phi*EndoCons(ai) / yh(age, lambdai,ei) )              
 
                      EndoYgrid(ai) =  agrid(ai) +  EndoCons(ai)   &
                                          &             - yh(age, lambdai,ei)* EndoHours(ai)
                 ENDDO ! ai  
                ...
                ENDDO
                 ...
                 DO ai=tempai,na               
                         ...
                         Hours(age, ai, zi, lambdai,ei) = max(0.0_DP, &
                                        &   1.0_DP-phi*Cons(age,ai,zi,lambdai,ei) / yh(age, lambdai,ei) )                                
                         ...
                         If   (Aprime(age, ai, zi, lambdai,ei)  .lt. amin) then
                               ...
                               Hours(age, ai, zi, lambdai,ei)  = max( 1.0_DP/(1.0_DP+phi) - &
                                & phi*( YGRID(ai,zi) - Aprime(age, ai, zi, lambdai,ei))/( (1.0_DP+phi)*yh(age, lambdai,ei)),0.0_DP)       
...
DO lambdai=1,nlambda        
        RetY(lambdai) = phi_lambda(lambdai) * Ebart 
ENDDO
...
DO lambdai=1,nlambda
      IF ((lambdagrid(lambdai)/lambdaBAR) .le. 0.3_DP) THEN
          phi_lambda(lambdai) = 0.9_DP*  (lambdagrid(lambdai)/lambdaBAR)
          ELSEIF   ((lambdagrid(lambdai)/lambdaBAR) .le. 2.0_DP) THEN
	           phi_lambda(lambdai) = 0.27_DP +  0.32_DP*  (lambdagrid(lambdai)/lambdaBAR-0.3_DP) 
	           ELSEIF ((lambdagrid(lambdai)/lambdaBAR) .le. 4.1_DP) THEN
	                  phi_lambda(lambdai) = 0.81_DP +  0.15_DP*  (lambdagrid(lambdai)/lambdaBAR-2.0_DP)
                          ELSE
                                phi_lambda(lambdai) =1.1_DP	   
        ENDIF
ENDDO
...
! This subroutine compute consumptions at all exogenous grids PLUS
! Computes consumption (AnRetCTemp) and value function (AnRetValueTemp) at a particular grid point agej, zj, lambdaj, YTemp
! It computes over the lifecycle not only during RETIREMENT
SUBROUTINE ANALYTIC_SOLUTION_MU1(agej, zj, lambdaj, YTemp, AnRetCTemp, AnRetHoursTemp, AnRetValueTemp)
USE parameters
USE GLOBAL
IMPLICIT NONE
INTEGER :: aget, lambdat, zt, at , agej, lambdaj, zj, aj
!REAL(DP), DIMENSION(MaxAge,MaxAge) :: DiscountF
REAL(DP), DIMENSION(MaxAge) :: CumDiscountF, CumDiscountF2
REAL(DP), DIMENSION(MaxAge, nz, nlambda) :: LifetimeWage
REAL(DP), DIMENSION(nz) :: RZ
REAL(DP), DIMENSION(MaxAge, nz, nlambda) :: ValueConstant
REAL(DP) :: AnRetCTemp, AnRetValueTemp, YTemp, AnRetHoursTemp


DO zt=1,nz
      RZ(zt)  =  ( 1.0_DP + ( rr_bench * zgrid(zt)- DepRate ) *(1.0_DP-tauk_bench) )*(1.0_DP-tauw_bench)
      LifetimeWage(MaxAge,zt,:) = RetY
ENDDO
!print*,'RZ*beta',RZ*beta

CumDiscountF(MaxAge) = 1.0_DP
CumDiscountF2(MaxAge) = 1.0_DP
ValueConstant(MaxAge,:,:) =0.0_DP

DO aget=MaxAge-1,RetAge,-1
   
    CumDiscountF(aget)   = 1.0_DP + beta * survP(aget) *CumDiscountF(aget+1)    
    CumDiscountF2(aget) = 1.0_DP + beta * survP(aget) *CumDiscountF(aget+1)    
    
    DO zt=1,nz
        DO lambdat=1,nlambda       
            LifetimeWage(aget,zt,lambdat) =  RetY(lambdat) +  LifetimeWage(aget+1,zt,lambdat) /  RZ(zt)
    
            ValueConstant(aget,zt,lambdat) = - CumDiscountF2(aget) * log(CumDiscountF2(aget)) &
                                             & + beta * survP(aget) * CumDiscountF(aget+1)       & 
                                             &  * log( beta * survP(aget) * CumDiscountF(aget+1) *RZ(zt) ) &
                                             & + beta * survP(aget) * ValueConstant(aget+1,zt,lambdat)         
        ENDDO
    
    ENDDO   
ENDDO

!print*,'LifetimeWage(:,1,1)='
!print*, LifetimeWage(:,1,1) 
!print*,'CumDiscountF='
!print*,CumDiscountF
!pause

!	Note that the analytical solution can be obtained only when ne=1. so, I am evaluating labor efficiency at ei=1
DO aget=RetAge-1,1,-1
    CumDiscountF(aget)   = 1.0_DP +          beta * survP(aget) *CumDiscountF(aget+1)     
    CumDiscountF2(aget) = 1.0_DP + phi + beta * survP(aget) *CumDiscountF2(aget+1)    
    
    DO zt=1,nz
        DO lambdat=1,nlambda       
            LifetimeWage(aget,zt,lambdat) =  yh(aget,lambdat,1) +  LifetimeWage(aget+1,zt,lambdat) /  RZ(zt)

            if (phi .gt. 0.0_DP) then            
                ValueConstant(aget,zt,lambdat) = - CumDiscountF2(aget) * log(CumDiscountF2(aget)) &
                                             & + beta * survP(aget) * CumDiscountF2(aget+1)       & 
                                             &  * log( beta * survP(aget) * CumDiscountF2(aget+1) *RZ(zt) ) & 
                                             & + beta * survP(aget) * ValueConstant(aget+1,  zt,lambdat)   &
                                             & + phi*log(phi/yh(aget,lambdat,1))     
            else
               ValueConstant(aget,zt,lambdat) = - CumDiscountF2(aget) * log(CumDiscountF2(aget)) &
                                             & + beta * survP(aget) * CumDiscountF(aget+1)       & 
                                             &  * log( beta * survP(aget) * CumDiscountF(aget+1) *RZ(zt) ) & 
...
! This is the consumption decision on the exogenous grid
DO aget=MaxAge,RetAge,-1
DO at=1,na
DO zt=1,nz
DO lambdat=1,nlambda

    AnRetCons(aget,at,zt,lambdat) = ( YK(at,zt) + LifetimeWage(aget, zt, lambdat) ) /   CumDiscountF2(aget)
    AnRetValue(aget,at,zt,lambdat) = CumDiscountF2(aget) * log(YK(at,zt) + LifetimeWage(aget, zt, lambdat)) &
                & + ValueConstant(aget,  zt,lambdat) 
    
ENDDO
ENDDO
ENDDO
!print*,'age',age,'value constant', ValueConstant(age,  :)
ENDDO
...
DO lambdat=1,nlambda
    ...
    AnRetCons(aget,at,zt,lambdat) = ( YK(at,zt) + LifetimeWage(aget, zt, lambdat) ) /   CumDiscountF2(aget)
    AnRetHours(aget,at,zt,lambdat)     =  1.0_DP-phi*AnRetCons(aget,at,zt,lambdat)/yh(aget,lambdat,1)
    AnRetValue(aget,at,zt,lambdat) = CumDiscountF2(aget) * log(YK(at,zt) &
                & + LifetimeWage(aget, zt, lambdat)) + ValueConstant(aget, zt, lambdat) 
    
ENDDO
ENDDO
ENDDO
!print*,'age',age,'CumDiscountF2(age)= ',CumDiscountF2(age) ,'CumDiscountF(age)=',CumDiscountF(age)
!pause
ENDDO
!pause

AnRetCTemp = (YTemp + LifetimeWage(agej, zj, lambdaj) ) /   CumDiscountF2(agej)

if (agej .lt. RetAge) then
    AnRetHoursTemp = 1.0_DP-phi*AnRetCTemp/yh(agej,lambdaj,1)
endif

AnRetValueTemp = CumDiscountF2(agej) * log(YTemp + LifetimeWage(agej, zj, lambdaj)) + ValueConstant(agej,  zj, lambdaj) 
