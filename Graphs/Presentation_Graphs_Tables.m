...
% Aggregate variabls as a function of capital/wealth tax revenue
    clf
    hold on
    plot(Stats_by_tau_k(:,4), 100*(Stats_by_tau_k(:,5)/Stats_by_tau_k(1,5)-1),'r')
    plot(Stats_by_tau_w(:,4), 100*(Stats_by_tau_w(:,5)/Stats_by_tau_w(1,5)-1),'b')
    plot(Stats_by_tau_k(:,4), 100*(Stats_by_tau_k(:,6)/Stats_by_tau_k(1,6)-1),'r-d')
    plot(Stats_by_tau_w(:,4), 100*(Stats_by_tau_w(:,6)/Stats_by_tau_w(1,6)-1),'b-d')
    plot(Stats_by_tau_k(:,4), 100*(Stats_by_tau_k(:,7)/Stats_by_tau_k(1,7)-1),'ro')
    plot(Stats_by_tau_w(:,4), 100*(Stats_by_tau_w(:,7)/Stats_by_tau_w(1,7)-1),'bo')
    plot(Stats_by_tau_k(:,4), 100*(Stats_by_tau_k(:,8)/Stats_by_tau_k(1,8)-1),'rd')
    plot(Stats_by_tau_w(:,4), 100*(Stats_by_tau_w(:,8)/Stats_by_tau_w(1,8)-1),'bd')
    grid on
    hold off
...
% Wage by capital/Wealth tax revenue
    clf
    hold on
    plot(Stats_by_tau_k(:,4), (Stats_by_tau_k(:,10)),'r')
    plot(Stats_by_tau_w(:,4), (Stats_by_tau_w(:,10)),'b')
    grid on
    hold off
    legend('\tau_K', '\tau_W')
    xlabel('GBAR_K')
...
% Average utilities (relavite to no taxes)
    clf
    hold on
    plot(Stats_by_tau_k(:,4), (Stats_by_tau_k(:,11)-Stats_by_tau_k(1,11)),'r')
    plot(Stats_by_tau_w(:,4), (Stats_by_tau_w(:,11)-Stats_by_tau_w(1,11)),'b')
    grid on; hold off
    xlabel('GBAR_K')
...
Stats_by_tau_k(:,13) =100*( (Stats_by_tau_k(:,11)./Stats_by_tau_k(26,11)).^(1/(gamma*(1-sigma)))-1);
Stats_by_tau_w(:,13) =100*( (Stats_by_tau_w(:,11)./Stats_by_tau_k(26,11)).^(1/(gamma*(1-sigma)))-1);

% CE Newborn computed using average utilities (relavite to no taxes)
    clf
    hold on
    plot(Stats_by_tau_k(:,4), (Stats_by_tau_k(:,13)-Stats_by_tau_k(1,13)),'r')
    plot(Stats_by_tau_w(:,4), (Stats_by_tau_w(:,13)-Stats_by_tau_w(1,13)),'b')
    grid on; hold off;
    xlabel('GBAR_K')
    title('CE Newborn computed using average utilities')

    [maxvalK indxK] = max((Stats_by_tau_k(:,13)-Stats_by_tau_k(1,13)));
    text(Stats_by_tau_k(indxK,4)+0.01, maxvalK -0.10,['Opt. \tau_K = ' ,num2str(100*Stats_by_tau_k(indxK,1)),'%'])

    [maxvalW indxW] = max((Stats_by_tau_w(:,13)-Stats_by_tau_w(1,13)));
    text(Stats_by_tau_w(indxW,4)+0.0, maxvalW +0.05,['Opt. \tau_W = ' ,num2str(100*Stats_by_tau_w(indxW,2)),'%'])
...
% CE Newborn computed using average utilities (in levels)
    clf
    hold on
    plot(Stats_by_tau_k(:,4), (Stats_by_tau_k(:,13) ),'r')
    plot(Stats_by_tau_w(:,4), (Stats_by_tau_w(:,13) ),'b')
    grid on; hold off;
    xlabel('GBAR_K')
    title('CE Newborn computed using average utilities')

    [maxvalK indxK] = max((Stats_by_tau_k(:,13) ));
    text(Stats_by_tau_k(indxK,4)+0.01, maxvalK -0.10,['Opt. \tau_K = ' ,num2str(100*Stats_by_tau_k(indxK,1)),'%'])

    [maxvalW indxW] = max((Stats_by_tau_w(:,13) ));
    text(Stats_by_tau_w(indxW,4)+0.0, maxvalW +0.05,['Opt. \tau_W = ' ,num2str(100*Stats_by_tau_w(indxW,2)),'%'])
...
% 
    GBAR_bench   = 0.292297160745428;
    SSC_Payments = 0.20878464352822504;
    TOT_budget   = GBAR_bench+SSC_Payments;
% 
    clf
    hold on
    plot(Stats_by_tau_k(:,4)/TOT_budget, (Stats_by_tau_k(:,13)-Stats_by_tau_k(1,13)),'r')
    plot(Stats_by_tau_w(:,4)/TOT_budget, (Stats_by_tau_w(:,13)-Stats_by_tau_w(1,13)),'b')
    grid on
    xlabel('GBAR_K / (GBAR+SSC)')
    title('CE Newborn computed using average utilities')

    [maxvalK indxK] = max((Stats_by_tau_k(:,13)-Stats_by_tau_k(1,13)));
    text(Stats_by_tau_k(indxK,4)/TOT_budget+0.01, maxvalK -0.10,['Opt. \tau_K = ' ,num2str(100*Stats_by_tau_k(indxK,1)),'%'])

    [maxvalW indxW] = max((Stats_by_tau_w(:,13)-Stats_by_tau_w(1,13)));
    text(Stats_by_tau_w(indxW,4)/TOT_budget+0.0, maxvalW +0.05,['Opt. \tau_W = ' ,num2str(100*Stats_by_tau_w(indxW,2)),'%'])
...
% 
    clf
    hold on
    plot(Stats_by_tau_k(:,4)/TOT_budget, (Stats_by_tau_k(:,13) ),'r')
    plot(Stats_by_tau_w(:,4)/TOT_budget, (Stats_by_tau_w(:,13) ),'b')
    grid on; hold off;
    xlabel('GBAR_K / (GBAR+SSC)')
    title('CE Newborn computed using average utilities')
