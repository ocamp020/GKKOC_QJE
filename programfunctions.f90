...
SUBROUTINE  INITIALIZE()
	...
	! Initiliaze grids for z, lamda and e	
		...
		if (Gz.eq.Gz_aux) then print*, 'Gz check' endif 
		if (zgrid.eq.zgrid_aux) then print*, 'zgrid check' endif 
		if (pr_z.eq.pr_z_aux) then print*, 'pr_z check' endif 
...
SUBROUTINE  SIMULATION_TOP(bench_indx)
	...
	!!!!! mu=0.95
	top_ind = (/10563514 , 15606788 , 13548896 , 16730950 , 16766492 , 11328153 , 18525999 , 12531737 , &
