...
SUBROUTINE GOVNT_BUDGET(print_flag)
	...
	print '(A,F7.3,X,X,A,F7.3)', ' 		Total Capital Income=', Tot_Cap_Inc , 'Algebra=', &
				& alpha*YBAR - DepRate*sum( sum(sum(sum(sum(sum(DBN1,6),5),4),3),1)*agrid )
	...
	DO WHILE ( ( DBN_dist .ge. DBN_criteria ) .and. ( simutime .le. 700 ) )
	    ...
	    IF (iter_indx .ge. update_period) THEN
	        ...
	        if (A_C.gt.0.0_dp) then 
		    	...
		    	if (R_C.lt.R) then 
		    		print '(A,F7.4,X,A,F7.4,X,A,F7.4,X,A,F7.4,X)',&
		    			&' 	Interest Rates Not in Order:  R=',100.0_dp*R,'R_C=',100.0_dp*R_C,'K_C=',K_C,'K_P=',K_P!,'brentvalue',brent_value
