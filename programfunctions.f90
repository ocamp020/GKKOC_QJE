	...
	DO WHILE ( ( DBN_dist .ge. DBN_criteria ) .and. ( simutime .le. 700 ) )
	    ...
	    IF (iter_indx .ge. update_period) THEN
	    	...
	    	if (sum(theta)/nz .gt. 1.0_DP) then
	    		P = min(P,1.0_dp)
	            brent_value = brent(-0.1_DP,0.01_DP,1.0_DP,Agg_Debt, brent_tol,R)
            else
            	print*,' '
            	print*, 'No Capital Market: Setting R=0'
            	print*, 'Theta=',theta,sum(theta)/nz
            	print*,' '
                R = 0.0_DP
	        endif
...
SUBROUTINE  INITIALIZE()
	...
	! Transitory investment productivity x
		if (nx.gt.1) then
			...
			! 	enddo
			! 	! print*, 'xgrid error'
			print*, 'Code hardwired for nx=1, if nx>1 uncomment the lines above.'
			STOP 
		else 
			xgrid = 1.0_dp
			xz_grid(1,:) = zgrid 
