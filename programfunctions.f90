...
!
	FUNCTION FOC_R(aprimet,state)
		...
		FOC_R	= ( (YGRID_t(a_in,z_in,x_in)+RetY_lambda_e(l_in,e_in)-aprimet) + Bequest   &
	...
	FUNCTION FOC_R_Transition(aprimet,state)
		...
		! Evaluate square residual of Euler equation at current state (given by (ai,zi,lambdai,ei)) and savings given by a'
		FOC_R_Transition	= ( (YGRID_t(a_in,z_in,x_in)+RetY_lambda_e(l_in,e_in)-aprimet) + Bequest    &
		        & - ( beta*survP(age_in) * sum( pr_x(x_in,:,z_in,age_in) * MB_aprime * cprime**(1.0_dp/euler_power) ) & 
		        & + (1.0_dp-survP(age_in))*(1.0_dp+tauC)**((1.0_dp-sigma)*gamma)*chi_bq* &
		        &				  ((1.0_dp-tau_bq)*aprimet+bq_0)**(1.0_dp/euler_power) ) &
		        & **euler_power) ** 2.0_DP
		...
		DO ai=tempai,na_t 
		    ...
		    If (Aprime_t(age, ai, zi, lambdai,ei,xi)  .lt. amin) then
		        ...
		        if (NSU_Switch.and.(Progressive_Tax_Switch.eqv..false.)) then 
		        	Hours_t(age, ai, zi, lambdai,ei,xi)  = max( 0.0_dp , gamma - (1.0_DP-gamma) *( & 
		        		& YGRID_t(ai,zi,xi) + Bequest - Aprime_t(age, ai, zi, lambdai,ei,xi)) / (psi*yh(age, lambdai,ei)) )
                else               	        
...
SUBROUTINE FIND_DBN_EQ()
	...
	REAL(DP), DIMENSION(:,:,:,:,:,:), allocatable ::  PrAprimelo, PrAprimehi, DBN2, DBN_death !,PrBqlo, PrBqhi,
	...
	DO WHILE ( ( DBN_dist .ge. DBN_criteria ) .and. ( simutime .le. 700 ) )
	    ...
	    DO e1=1, ne
	        ...
	        DO lambda2=1,nlambda
	        	DBN2(1, 1, z2, lambda2, ne/2+1, 1) =  DBN2(1, 1 ,z2,lambda2,ne/2+1,1) + DBN1(age1,a1,z1,lambda1,e1,x1) &
	    ...
	    IF (iter_indx .ge. update_period) THEN
	    	...
	    	print 12345, &
	    		...
	    		& 'K_C/A=',100.0_dp*K_C/Wealth,'L_C/N=',100.0_dp*L_C/NBAR,&
	    		& 'B/A=',Bequest/(sum( sum(sum(sum(sum(sum(DBN1,6),5),4),3),1)*agrid )),'Bequest=',Bequest
    		12345 format &
