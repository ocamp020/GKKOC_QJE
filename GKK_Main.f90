	...
	! Set Parameters
		...
		! Debt/Output = 1.5, lambda = 1.5, no bequest fee
			! Main Parameters 
				...
				sigma_z_eps      = 0.100_dp ! 0.0867_dp (Value without estate tax) ! 0.072_dp (value in old benchmark) ! params(4) !
			...
			! Bequeset parameters chi_bq*(bq+bq_0)^(1-sigma)
				...
				chi_u  = 03.00_dp ! Scaling 03.55_dp (value without estate tax)
				chi_bq = chi_u*(1.0_dp-tau_bq) ! Auxiliary parameter for FOC and EGM

			! Capital Market
				do zi=1,nz
				theta(zi)    = 1.00_dp+(3.40_dp-1.00_dp)/(nz-1)*(real(zi,8)-1.0_dp)
				enddo
