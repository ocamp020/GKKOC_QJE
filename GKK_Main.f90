...
PROGRAM main
	...
	! Set Parameters
		...
		! Debt/Output = 1.5, lambda = 1.5, no bequest fee
			...
			! Omega transfer to retirees (target total transfers to GDP equal to 4.9%)
			omega_ret = 0.049_dp*(1-0.2517_dp)/((1.0_dp-alpha)*0.2517_dp)
				! omega_ret*EBAR*RET/GDP = (4.9*(1-RET)*EBAR/((1-a)*RET))*RET/GDP = (4.9*(1-RET)*(N*W/(1-RET))/((1-a)*GDP)

			! Awesome State Level
			awesome_level = 265.0_dp 

		else

		! Corporate Sector
		print*,"You are entering the corporate sector parameters and you shouldn't be here. Stop required."
		stop 
	...
	if (A_C.eq.0.0_dp) then 
 		if ((Progressive_Tax_Switch.eqv..false.).and.(NSU_Switch.eqv..true.)) then 
			Result_Folder = './Revision/Model_2.1_Super_Aiyagari/'
...
Subroutine Solve_Benchmark(compute_bench,Simul_Switch)
	...
	if (compute_bench) then
		...
		CALL FIND_DBN_EQ_Infinite_Horizon
		print*,"	Computing government spending"
		CALL GOVNT_BUDGET_Infinite_Horizon(.true.)
	...
	! Aggregate variables in benchmark economy
		...
		Cons_bench          = Cons_ih
		Hours_bench         = Hours_ih
		Aprime_bench        = Aprime_ih
		...
		CALL COMPUTE_STATS_Infinite_Horizon
		...
		if (Simul_Switch) then 
			...
			CALL Simulation_Infinite_Horizon(solving_bench)
		 		print*, '	Sleeping for 5min before simulating dyansties'
				call sleep(300)
			CALL Simulation_Infinite_Horizon_Dynasties(solving_bench)
		endif
