	...
	! Set Parameters
		...
		! Debt/Output = 1.5, lambda = 1.5, no bequest fee
			! Main Parameters 
				...
				sigma_z_eps      = 0.069_dp ! 0.0867_dp (Value without estate tax) ! 0.072_dp (value in old benchmark) ! params(4) !
			...
			! Capital Market
				do zi=1,nz
				theta(zi)    = 1.00_dp+(3.5_dp-1.00_dp)/(nz-1)*(real(zi,8)-1.0_dp)
				enddo
...
Subroutine Solve_Experiment(compute_exp,Simul_Switch)
	...
	! Set Y_a_threshold
		...
		tauWmin_at=0.023_DP
		tauWinc_at=0.0002_DP ! Minimum tax above threshold and increments
