	...
	! Set Parameters
		...
		! Debt/Output = 2.0
			! Main Parameters 
				...
				sigma_z_eps      = 0.0705_dp ! 0.0867_dp (Value without estate tax) ! 0.072_dp (value in old benchmark) ! params(4) !
				sigma_lambda_eps = 0.318_dp ! 0.309_dp (Value without estate tax) ! 0.305_dp (value in old benchmark) ! params(5)
				gamma  	=  0.4445_dp ! 0.4580_dp (Value without estate tax) ! 0.46_dp (value in old benchmark) !  params(6) ! 

			! Bequeset parameters chi_bq*(bq+bq_0)^(1-sigma)
				bq_0   = 00.30_dp ! Level shift 00.30_dp (value without estate tax)
				chi_u  = 00.55_dp ! Scaling 03.55_dp (value without estate tax)
				chi_bq = chi_u*(1.0_dp-tau_bq) ! Auxiliary parameter for FOC and EGM

			! Capital Market
				do zi=1,nz
				theta(zi)    = 1.00_dp+(5.40_dp-1.00_dp)/(nz-1)*(real(zi,8)-1.0_dp)
				enddo
