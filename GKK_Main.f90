...
PROGRAM main
	...
	! Set Parameters
		...
		! Debt/Output = 1.5, lambda = 1.5, no bequest fee
			! Main Parameters 
				beta   	= 0.9520_dp ! 0.9404_dp (Value without estate tax)! 0.9475_dp (value in old benchmark) ! params(1) !
				sigma_z_eps      = 0.103_dp ! 0.0867_dp (Value without estate tax) ! 0.072_dp (value in old benchmark) ! params(4) !
				sigma_lambda_eps = 0.280_dp ! 0.309_dp (Value without estate tax) ! 0.305_dp (value in old benchmark) ! params(5)
				gamma  	= 0.465_dp ! 0.4580_dp (Value without estate tax) ! 0.46_dp (value in old benchmark) !  params(6) ! 
				sigma  	= 4.0_dp
				x_hi	= 3.00_dp

			! Bequeset parameters chi_bq*(bq+bq_0)^(1-sigma)
				bq_0   = 00.30_dp ! Level shift 00.30_dp (value without estate tax)
				chi_u  = 02.50_dp ! Scaling 03.55_dp (value without estate tax)
				chi_bq = chi_u*(1.0_dp-tau_bq) ! Auxiliary parameter for FOC and EGM

			! Capital Market
				do zi=1,nz
				theta(zi)    = 1.00_dp+(3.15_dp-1.00_dp)/(nz-1)*(real(zi,8)-1.0_dp)
				enddo
	...
	if (A_C.eq.0.0_dp) then 
 		if ((Progressive_Tax_Switch.eqv..false.).and.(NSU_Switch.eqv..true.)) then 
			Result_Folder = './Revision/Model_2.1_Match_Return_Lambda_Local/'
		else if ((Progressive_Tax_Switch.eqv..true.).and.(NSU_Switch.eqv..true.)) then 
			Result_Folder = './Revision/Model_2.1_Match_Return_Lambda_PT/' 
		else if ((Progressive_Tax_Switch.eqv..false.).and.(NSU_Switch.eqv..false.)) then 
			Result_Folder = './Revision/Model_2.1_Match_Return_Lambda_SU/' 
		else if ((Progressive_Tax_Switch.eqv..true.).and.(NSU_Switch.eqv..false.)) then 
			Result_Folder = './Revision/Model_2.1_Match_Return_Lambda_PT_SU/' 
		end if
	else 
 		if ((Progressive_Tax_Switch.eqv..false.).and.(NSU_Switch.eqv..true.)) then 
			Result_Folder = './Revision/Model_2.1_Match_Return_Lambda_Corp/' 
		else if ((Progressive_Tax_Switch.eqv..true.).and.(NSU_Switch.eqv..true.)) then 
			Result_Folder = './Revision/Model_2.1_Match_Return_Lambda_Corp_PT/' 
		else if ((Progressive_Tax_Switch.eqv..false.).and.(NSU_Switch.eqv..false.)) then 
			Result_Folder = './Revision/Model_2.1_Match_Return_Lambda_Corp_SU/' 
		else if ((Progressive_Tax_Switch.eqv..true.).and.(NSU_Switch.eqv..false.)) then 
			Result_Folder = './Revision/Model_2.1_Match_Return_Lambda_Corp_PT_SU/' 
		end if
	endif 
