...
PROGRAM main
	...
	! Set Parameters
		...
		! Debt/Output = 1.5, lambda = 1.5, no bequest fee
			! Main Parameters 
				beta   	= 0.9646_dp ! 0.9404_dp (Value without estate tax)! 0.9475_dp (value in old benchmark) ! params(1) !
				sigma_z_eps      = 0.2760_dp ! 0.0867_dp (Value without estate tax) ! 0.072_dp (value in old benchmark) ! params(4) !
	...
	if (A_C.eq.0.0_dp) then 
 		if ((Progressive_Tax_Switch.eqv..false.).and.(NSU_Switch.eqv..true.)) then 
			Result_Folder = './Revision/Model_2.1_Rev_Const/'
		else if ((Progressive_Tax_Switch.eqv..true.).and.(NSU_Switch.eqv..true.)) then 
			Result_Folder = './Revision/Model_2.1_Rev_Const_PT/' 
		else if ((Progressive_Tax_Switch.eqv..false.).and.(NSU_Switch.eqv..false.)) then 
			Result_Folder = './Revision/Model_2.1_Rev_Const_SU/' 
		else if ((Progressive_Tax_Switch.eqv..true.).and.(NSU_Switch.eqv..false.)) then 
			Result_Folder = './Revision/Model_2.1_Rev_Const_PT_SU/' 
		end if
	else 
 		if ((Progressive_Tax_Switch.eqv..false.).and.(NSU_Switch.eqv..true.)) then 
			Result_Folder = './Revision/Model_2.1_Rev_Const_Corp/' 
		else if ((Progressive_Tax_Switch.eqv..true.).and.(NSU_Switch.eqv..true.)) then 
			Result_Folder = './Revision/Model_2.1_Rev_Const_Corp_PT/' 
		else if ((Progressive_Tax_Switch.eqv..false.).and.(NSU_Switch.eqv..false.)) then 
			Result_Folder = './Revision/Model_2.1_Rev_Const_Corp_SU/' 
		else if ((Progressive_Tax_Switch.eqv..true.).and.(NSU_Switch.eqv..false.)) then 
			Result_Folder = './Revision/Model_2.1_Rev_Const_Corp_PT_SU/' 
		end if
	endif 
