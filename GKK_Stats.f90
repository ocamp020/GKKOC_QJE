...
SUBROUTINE COMPUTE_STATS_Infinite_Horizon()
	use omp_lib
	IMPLICIT NONE
	...
	!------------------------------------------------------------------------------------
		! print*, 'Test assets'
		DO ai=1,na
		     pr_a_dbn(ai)          = sum(DBN_ih(ai,:)) 
		     cdf_a_dbn(ai)         = sum( pr_a_dbn(1:ai) )      
		     tot_a_by_grid(ai)     = sum(DBN_ih(ai,:) * agrid(ai) )
	...
	!------------------------------------------------------------------------------------
	!------------------------------------------------------------------------------------
		! COMPUTE AVERAGE HOURS  INCLUDING NON-WORKERS
		! COMPUTE VARIANCE OF LOG EARNINGS THOSE WHO WORK MORE THAN 260 HOURS
		...
		DO ai=1,na
		DO ei=1,ne
			tothours_25_60 = tothours_25_60 + DBN_ih(ai,ei)  * Hours_ih(ai,ei)
			! pop_25_60      = pop_25_60 +  DBN_ih(ai,ei)
			IF (Hours_ih(ai,ei) .ge. 0.055) THEN
			tot_log_earnings_25_60 = tot_log_earnings_25_60 + DBN_ih(ai,ei)  &
			                 		& *  log( Y_h(Hours_ih(ai,ei),1,1,ei,wage) )
			pop_pos_earn_25_60     = pop_pos_earn_25_60 +  DBN_ih(ai,ei)
			ENDIF
		ENDDO
		ENDDO
		pop_25_60 = sum(DBN_ih(:,1:4))
		...
		DO ei=1,ne
			IF (Hours_ih(ai,ei) .ge. 0.055) THEN
			    Var_Log_Earnings_25_60 =  Var_Log_Earnings_25_60 + DBN_ih(ai,ei)  &
			                 			& * ( log( Y_h(Hours_ih(ai,ei),1,1,ei,wage) ) &
	...
	!------------------------------------------------------------------------------------
		...
		do ai=1,na 
		do ei=1,ne 
		MeanWealth   = MeanWealth   + DBN_ih(ai,ei)*agrid(ai)
		enddo 
		enddo 
	...
	!------------------------------------------------------------------------------------
		...
		DO ei=1, ne
		DO ai=1,na
		if (HOURS_ih(ai,ei).gt.0.001_dp) then 
		Size_Frisch = Size_Frisch + DBN_ih(ai,ei)
		Frisch_Elasticity = Frisch_Elasticity + DBN_ih(ai,ei)*(1.0_dp-tauPL)/ &
		& ( sigma/(1.0_dp-(1.0_dp-sigma)*gamma) * HOURS_ih(ai,ei)/(1-HOURS_ih(ai,ei)) - tauPL )
		Hours_Frisch = Hours_Frisch + DBN_ih(ai,ei)*HOURS_ih(ai,ei)
		endif 
		ENDDO
		ENDDO
	...
	print*, ' '; print*,' End of Compute_Stats Infinite Horizon'; print*, ' '


END SUBROUTINE COMPUTE_STATS_Infinite_Horizon
...
SUBROUTINE Write_Benchmark_Results(Compute_bench)
	...
	IF (Compute_bench) then 
		OPEN  (UNIT=1,  FILE=trim(bench_folder)//'cons_ih'  , STATUS='replace')
		WRITE (UNIT=1,  FMT=*) cons_ih
		CLOSE (unit=1)
		OPEN  (UNIT=2,  FILE=trim(bench_folder)//'aprime_ih', STATUS='replace')
		WRITE (UNIT=2,  FMT=*) aprime_ih
		CLOSE (unit=2)
		OPEN  (UNIT=3,  FILE=trim(bench_folder)//'hours_ih' , STATUS='replace')
		WRITE (UNIT=3,  FMT=*) hours_ih
		CLOSE (unit=3)

		OPEN  (UNIT=5,  FILE=trim(bench_folder)//'DBN_ih'   , STATUS='replace')
		WRITE (UNIT=5,  FMT=*) DBN_ih
		CLOSE (UNIT=5)
	...
	ELSE 
		OPEN (UNIT=1,  FILE=trim(bench_folder)//'cons_ih'  , STATUS='old', ACTION='read')
		OPEN (UNIT=2,  FILE=trim(bench_folder)//'aprime_ih', STATUS='old', ACTION='read')
		OPEN (UNIT=3,  FILE=trim(bench_folder)//'hours_ih' , STATUS='old', ACTION='read')
		OPEN (UNIT=5,  FILE=trim(bench_folder)//'DBN_ih'   , STATUS='old', ACTION='read')
		...
		READ (UNIT=1,  FMT=*) cons_ih
		READ (UNIT=2,  FMT=*) aprime_ih
		READ (UNIT=3,  FMT=*) hours_ih
		READ (UNIT=5,  FMT=*) DBN_ih
		...
		CLOSE (unit=1); CLOSE (unit=2); CLOSE (unit=3); CLOSE (unit=5)
		...
		CLOSE (unit=16); CLOSE (unit=17); CLOSE (unit=18); 
		CLOSE (unit=21); 
