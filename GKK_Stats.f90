...
SUBROUTINE COMPUTE_STATS()
	...
	!------------------------------------------------------------------------------------ 
		...
		!! All active agents (z>0)
			...
			Av_Mg_Ret_K = sum(MgPr_K_mat*DBN_azx_aux)
			SD_Mg_Ret_K = sqrt( sum( ((MgPr_K_mat - Av_Mg_Ret_K)**2.0_dp)*DBN_azx_aux ) )
			...
			do i=1,size(prctile_bq)
				...
				b = maxval(MgPr_K_vec)
				c = (a+b)/2.0_dp
				CCDF_c = sum(DBN_azx_aux_vec,MgPr_K_vec>=c)
				!print*, ' '
				!print*, 'Percentile', prctile_bq(i)
				do while ((abs(CCDF_c-prctile_bq(i))>0.00001_dp).and.(b-a>1e-9))
					if (CCDF_c<prctile_bq(i)) then 
			...
			Av_Mg_Ret_K_W = sum( MgPr_K_mat*DBN_azx_aux )
			SD_Mg_Ret_K_W = sqrt( sum( ((MgPr_K_mat - Av_Mg_Ret_K_W)**2.0_dp)*DBN_azx_aux ) )
		...
		!! Entrepreneurs (business income/total income > 50%)
			...
			!! Weighted 
			Av_Mg_Ret_K_W = sum(MgPr_K_mat*DBN_azx_W_aux)
			SD_Mg_Ret_K_W = sqrt( sum( ((MgPr_K_mat - Av_Mg_Ret_K_W)**2.0_dp)*DBN_azx_W_aux ) )
			...
			do i=1,size(prctile_bq)
				...
				b = maxval(MgPr_K_vec)
				c = (a+b)/2.0_dp
				CCDF_c = sum(DBN_azx_W_aux_vec,MgPr_K_vec>=c)
				!print*, ' '
				!print*, 'Percentile', prctile_bq(i)
				do while ((abs(CCDF_c-prctile_bq(i))>0.00001_dp).and.(b-a>1e-9))
					if (CCDF_c<prctile_bq(i)) then 
	...
	!------------------------------------------------------------------------------------
	do age=1,MaxAge 
		Wealth_age(age) = sum(agrid*sum(sum(sum(sum(DBN1(age,:,:,:,:,:),5),4),3),2))/sum(DBN1(age,:,:,:,:,:))
